<template>
    <div id="usercollect">
        <ul class="media-list">
            <li v-for="(item , id) in collectList" :key="id" class="list-shadow list">
                <el-row>
        ...
        <div v-if="isShow" style="text-align: center">
            ...
            <p style="font-weight: bolder">暂时没收藏喔~</p>
        </div>
...
<script>
  ...
  import {getUserCollect} from "../../../service/api";
  ...
  export default {
    ...
    methods: {
      async getCollectList() {
        let res = await getUserCollect(this.userInfo.uId, this.pageNum, this.currentPage);
